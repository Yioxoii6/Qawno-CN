...
中文 Pawn 脚本（GBK/GB2312）打开后中文注释与字符串正常显示，不再乱码；
注意顶部菜单栏也已是中文（文件 / 编辑 / 编译 / 设置 / 帮助）。

![正常读取 GBK 源码](docs/截图/1-正常读取GBK源码.png)

### 汉化右键菜单

代码编辑区的**右键菜单**已全部汉化（撤销 / 重做 / 剪切 / 复制 / 粘贴 / 删除 / 全选）。
这一处光靠 Qt 官方 `qt_zh_CN.qm` 是治不了的，必须改代码 —— 原因见
[`docs/汉化说明.md`](docs/汉化说明.md) 的「右键菜单为什么要自己改代码」。

![汉化右键菜单](docs/截图/2-汉化右键菜单.png)

### 编译输出区

输出区与跳转错误同样为中文（如 `下一个错误（N） Ctrl+E`）。

![编译输出区](docs/截图/3-正常读取GBK源码-编译输出.png)
